...
Tool + UI resource metadata
tools/call (render_* payload)
Tool result</br>payload + ui.resourceUri
Tool result
resources/read (ui://...)
UI HTML
Mount view + inject host styles
ui/initialize
ui/notifications/tool-input + hostContext
ui/notifications/size-changed
Updated view + host styles
Interaction</br>(e.g. click, hover)
ui/message + payload
Forward to tool loop
User
...
What I Built
For the lab, I used my bEvr monorepo template2 to build a simple MCP server and three Plot-based widgets: bar chart, line chart, and scatterplot. The server exposes each widget as a UI resource and a render tool, and the widgets connect to the host for theming and event streaming.
...
Lessons Learned
MCP Apps make UI first-class. It's the missing layer between chat tools and real interaction.
Schema reuse is a superpower. Sharing the payload schema between server + widget keeps the UI contract tight.
Theming needs to be host-driven. CSS variables give you a real white-label surface without rebuilding components.
Conclusion
